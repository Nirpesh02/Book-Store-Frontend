...
export default function AddBookModal({ isOpen, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast(`"${formData.title}" added to inventory!`, 'success');
      onClose();
      setFormData({ title: '', author: '', category: 'Fiction', isbn: '', price: '', discount: 0, copies: 1, coverImages: [], description: '' });
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
      <div className="bg-white w-full max-w-lg rounded-3xl p-6 shadow-2xl relative border border-slate-100 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-slate-400">Add a new book to the store inventory</p>
        </div>

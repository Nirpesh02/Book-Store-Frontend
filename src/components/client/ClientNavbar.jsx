...
import { Store, Search, ShoppingBag, Heart, User, ShieldAlert, LogOut, X, ShoppingCart, ShieldCheck, Menu, Bell } from 'lucide-react';
...
export default function ClientNavbar({ clientTab, setClientTab, onLogout, currentUser, onSwitchToAdmin }) {
  ...
  const notifications = currentUser?.notifications ? [...currentUser.notifications].reverse() : [];
  const [unreadNotifsCount, setUnreadNotifsCount] = useState(0);

  useEffect(() => {
    setUnreadNotifsCount(notifications.filter(n => !n.isRead).length);
  }, [currentUser]);

  const handleNotificationsClick = async () => {
    setNotificationsOpen(!notificationsOpen);
    if (!notificationsOpen && unreadNotifsCount > 0) {
      try {
        await authAPI.markNotificationsRead();
        setUnreadNotifsCount(0); // Optimistically update UI
      } catch (err) {
                ...
                {notificationsOpen && (
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-xl border border-slate-200 z-50 overflow-hidden flex flex-col">
                    <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
                      ...
                      {unreadNotifsCount > 0 && (
                        <span className="text-[10px] font-bold bg-rose-100 text-rose-600 px-2 py-0.5 rounded-md">
                    ...
                    <div className="max-h-[300px] overflow-y-auto">
                      {notifications.length > 0 ? (
                        notifications.map((notif, idx) => (
                          <div key={idx} className={`p-4 border-b border-slate-50 last:border-0 hover:bg-slate-50 transition-colors ${!notif.isRead ? 'bg-amber-50/30' : ''}`}>
